...
Create a server that logs all incoming requests to a file named `requests.log`. A log entry contains only `LOG_LEVEL` and `LOG_MSG`. The log levels are: ["ERROR", "WARN", "INFO", "DEBUG"]. The server should listen on port 8080 and handle GET and POST requests. Implement this endpoints:
